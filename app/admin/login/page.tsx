...
export default function AdminLoginPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const { token } = await adminLogin(String(form.get('username')), String(form.get('password')));
      setToken(token);
  ...
  return (
    <div className="mx-auto mt-20 max-w-md px-4">
      <form onSubmit={onSubmit} className="glass-card space-y-4 p-6">
        ...
        <input name="username" required placeholder="Username" className="w-full rounded-lg border border-white/10 bg-white/5 p-3" />
